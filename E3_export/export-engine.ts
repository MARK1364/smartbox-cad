/**
 * E3_export - export-engine.ts
 * Zarządca stanu eksportu, zrzutów 3D z precyzyjnym kadrowaniem (Cropping),
 * wirtualnego arkusza HUD na scenie 3D (z tabelką ISO 7200) oraz zapisanych arkuszy dla modułu Eksport 3.
 */
...
const SAVED_VIEWS_STORAGE_KEY = 'smartbox_cad_manual_sheets_v3_e3';
...
export class ExportEngineV3 {
    private static _instance: ExportEngineV3;

    public paperFormat: PaperFormat = 'A4_LANDSCAPE';
    public projectionType: ProjectionType = 'ORTHO';
    public renderStyle: ExportRenderStyle = 'technical';
    ...
    public static get instance(): ExportEngineV3 {
        if (!ExportEngineV3._instance) {
            ExportEngineV3._instance = new ExportEngineV3();
        }
        return ExportEngineV3._instance;
    }
    ...
    constructor() {
        ...
        if (typeof window !== 'undefined') {
            window.addEventListener('resize', () => {
                if (this.showBounds && ContextManager.instance?.activeTab === 'tab-e3-export') {
    ...
    public calculateSheetLayout(): SheetScreenLayout {
        ...
        const scale = sheetPixelW / paperW;
    ...
    public updatePassepartout(): void {
        const isExportTab = ContextManager.instance?.activeTab === 'tab-e3-export';
        ...
        if (!this._passepartoutElement) {
            ...
            this._passepartoutElement.id = 'export-virtual-sheet-hud-e3';
        ...
        this._passepartoutElement.innerHTML = `
            <svg width="100%" height="100%" viewBox="0 0 ${winW} ${winH}" style="display:block; width:100vw; height:100vh;">
                ...
                <rect x="0" y="0" width="${winW}" height="${winH}" fill="rgba(15, 23, 42, 0.72)" mask="url(#passepartout-mask-e3)" />
                ...
                <g transform="translate(${tbx.toFixed(1)}, ${tby.toFixed(1)})">
                    ...
                    <text x="${(tbw - 3).toFixed(1)}" y="${(tbh - 2).toFixed(1)}" font-size="${(1.2 * scale).toFixed(1)}" fill="#94a3b8" text-anchor="end" font-family="'Segoe UI', sans-serif">SmartBox CAD E3</text>
                </g>
            </svg>
        `;
    }
        ...
        try {
            ...
            if (typeof BABYLON !== 'undefined' && BABYLON.Tools?.CreateScreenshotAsync && engine && camera) {
                try {
                    ...
                    console.warn('CreateScreenshotAsync ostrzeżenie:', bjsErr);
    ...
    public projectPMIToSvg(sheet: DrawingSheet): void {
        ...
        try {
            ...
            console.warn('Błąd podczas rzutowania PMI:', err);
        }
    }
    ...
    public async saveCurrentViewWithThumbnail(customName?: string, description?: string): Promise<SavedExportView> {
        ...
        try {
            ...
            console.warn('Nie udało się przechwycić miniaturki:', e);
        }
    ...
    private loadSavedViews(): void {
        try {
            ...
            console.warn('Nie udało się wczytać zapisanych arkuszy E3:', e);
            this.savedViews = [];
        }
    }
    ...
    private saveSavedViews(): void {
        try {
            ...
            console.warn('Nie udało się zapisać arkuszy E3:', e);
        }
    }
}
