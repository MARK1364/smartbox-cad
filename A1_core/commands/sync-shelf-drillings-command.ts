/**
 * sync-shelf-drillings-command.ts
 *
 * Komenda aktualizująca nawiercenia półek i Systemu 32 na formatkach korpusu.
 * Zgodna z architekturą SyncBackGroovesCommand.
 */

import { ProjectDocument } from '../project-document.js';
import { Command } from './command.js';
import { buildShelvesDrillings } from '../../A2_smartbox/shelves-drilling-builder.js';

export class SyncShelfDrillingsCommand implements Command {
    readonly id: string;
    readonly label: string = 'Aktualizacja nawierceń półek';
    ...
    constructor(cabinetContainerId?: string) {
        this.id = `cmd_sync_shelf_drillings_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
    ...
    execute(document: ProjectDocument): void {
        ...
        // 1. Wylicz intencje nawierceń ze SmartBoxa
        const intents = buildShelvesDrillings(document, this.cabinetContainerId);

        // 2. Zidentyfikuj wszystkie formatki w dokumencie
        const panels = typeof document.getPanels === 'function' ? document.getPanels() : [];
        const changedNodeIds = new Set<string>();

        // 3. Wyczyść stare automatyczne nawiercenia półkowe (zachowaj inne obróbki)
        for (const panelNode of panels) {
            ...
            if (data && data.features && Array.isArray(data.features)) {
                ...
                data.features = data.features.filter((f: any) => {
                    ...
                    return !(f.type === 'hole' && f.params?.isShelfDrilling);
                });
