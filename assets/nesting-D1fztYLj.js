import{r as M,C as gt,j as e,c as zt,R as St}from"./context-manager-OvCG8LSO.js";import{N as q}from"./nesting-engine-SKvGxjPJ.js";import{a6 as At,i as Ct,r as Lt}from"./report-data-normalizer-BhIVhMxm.js";import{M as Pt}from"./ModuleSourceBar-CWyKYswQ.js";function Bt(s){const o=(s.name||"").trim(),r=(s.furnitureName||"").trim();if(r){let m=o;return m.toLowerCase().startsWith(r.toLowerCase())&&(m=m.slice(r.length).replace(/^\s*[-–—:|/]\s*/,"").trim()),{project:r,panel:m||o||"Formatka"}}const i=o.match(/^(.+?)\s*[-–—|/]\s*(.+)$/);return i?{project:i[1].trim(),panel:i[2].trim()}:{project:"",panel:o||"Formatka"}}class It{static generatePrintableHtml(o,r){const i=o.boards.map(n=>this.generateBoardSvgString(n,r)),m=o.machineType==="cnc"?"⚙️ Frezarka CNC (Nesting)":"🪚 Piła Formatowa / Panelowa";let d=`<!DOCTYPE html>
<html lang="pl">
<head>
    <meta charset="UTF-8">
