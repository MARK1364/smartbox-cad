/**
 * E1_export - drawing-sheet.ts
 * SmartBox CAD Drawing Sheet Generator (TypeScript / Web Port)
 * 
...
export class DrawingSheet {
    ...
    public async toRasterDataUrl(format: 'image/jpeg' | 'image/png' = 'image/jpeg', quality = 0.95, scale = 2.5): Promise<string> {
        ...
        // 2. Jeśli mamy zrzut z widoku 3D, rysujemy go bezpośrednio na canvasie w obszarze roboczym
        if (this._viewportPng) {
            const area = this.getAvailableDrawingArea();
    ...
    public static printMultiSheet(sheets: DrawingSheet[]): void {
        ...
        const pagesHtml = sheets
            .map((sheet, idx) => {
                ...
                return `
                    <div class="sheet-page" style="page-break-after: ${idx < sheets.length - 1 ? 'always' : 'auto'}; width: 100vw; height: 100vh; display: flex; align-items: center; justify-content: center;">
                        ${svg}
                    </div>
                `;
            })
        ...
        printWindow.document.write(`
            ...
            <html>
            <head>
                <title>Dokumentacja Rysunkowa CAD — SmartBox (${sheets.length} arkuszy)</title>
                <style>
                    @page {
                        size: auto;
                        margin: 0;
