import{r as M,C as gt,j as e,aE as zt,i as St,c as At,R as Ct,a as Lt}from"./report-data-normalizer-DzjLKfBX.js";import{N as q}from"./nesting-engine-PW0pZEE3.js";import{M as Pt}from"./ModuleSourceBar-BDudZ81u.js";function Bt(s){const o=(s.name||"").trim(),r=(s.furnitureName||"").trim();if(r){let m=o;return m.toLowerCase().startsWith(r.toLowerCase())&&(m=m.slice(r.length).replace(/^\s*[-–—:|/]\s*/,"").trim()),{project:r,panel:m||o||"Formatka"}}const i=o.match(/^(.+?)\s*[-–—|/]\s*(.+)$/);return i?{project:i[1].trim(),panel:i[2].trim()}:{project:"",panel:o||"Formatka"}}class It{static generatePrintableHtml(o,r){const i=o.boards.map(n=>this.generateBoardSvgString(n,r)),m=o.machineType==="cnc"?"⚙️ Frezarka CNC (Nesting)":"🪚 Piła Formatowa / Panelowa";let d=`<!DOCTYPE html>
<html lang="pl">
<head>
    <meta charset="UTF-8">
