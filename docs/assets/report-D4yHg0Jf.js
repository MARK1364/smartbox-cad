                ...
                <div class="cut-sheet">
                    ...
                    <table class="cut-header-table">
                        <tbody>
                            ...
                            <tr><th>Data</th><td>${i}</td><th>Strona</th><td>${_+1}/${z}</td></tr>
                        </tbody>
                    </table>
                    ...
                    <table class="cutting-table-a4">
                        ...
                        <tbody>`;for(let v=0;v<30;v++)if(v<N.length){const x=N[v],E=v+1,L=this.getEdgeCode(x.edge_config,"length",x.is_x_longer),B=this.getEdgeCode(x.edge_config,"width",x.is_x_longer);s+=`
                            <tr>
                                <td>${E}</td>
...
</html>`}generateCsv(t){const e=C.aggregateContractRows(t),n=[];n.push("\uFEFFLP;Dlugosc;Okleinowanie_Dlugosci;Szerokosc;Okleinowanie_Szerokosci;Szt;Rola;Material;Grubosc");let i=1;for(const a of Object.values(e)){const r=this.getEdgeCode(a.edge_config,"length",a.is_x_longer),l=this.getEdgeCode(a.edge_config,"width",a.is_x_longer),s=this.engine.resolveMaterialName(a.material);n.push([i++,Math.round(a.length_mm),r,Math.round(a.width_mm),l,a.qty,`"${a.role.replace(/"/g,'""')}"`,`"${s.replace(/"/g,'""')}"`,a.thickness_mm].join(";"))}return n.join(`
`)}downloadFile(t,e,n="text/html;charset=utf-8"){const i=new Blob([t],{type:n}),a=URL.createObjectURL(i),r=document.createElement("a");r.href=a,r.download=e,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(a)}openHtmlInNewTab(t){const e=new Blob([t],{type:"text/html;charset=utf-8"}),n=URL.createObjectURL(e);window.open(n,"_blank")||alert("Wyskakujące okno zostało zablokowane przez przeglądarkę.")}}P("report");function W(c){const t=new S,e=new Y(t),n=c.panels.map(l=>t.calculatePartPricing(l)),i=(c.accessories||[]).map(l=>t.calculateAccessoryPricing(l)),a=t.calculateGlobalSummary(n,i),r=c.scope?.type==="PROJECT"||!c.scope?.name?c.meta?.originLabel||"Projekt CAD":`Wycena: ${c.scope.name}`;return e.generateFullProjectReport(r,a,n,i)}function Z(c,t){const e=JSON.parse(c),n=e.panels?{meta:{module:"report",sourceId:"json",loadedAt:new Date().toISOString(),originLabel:`JSON · ${t}`},scope:e.scope||{type:"PROJECT",id:"ALL",name:t},panels:e.panels,accessories:e.accessories||[],furnitures:e.furnitures||[],containers:e.containers||[]}:e;if(!n.panels)throw new Error("JSON musi zawierać tablicę panels");return n}function q(){const[c,t]=w.useState(()=>T("report")),[e,n]=w.useState(c?null:"Brak danych. Otwórz raport z drzewa CAD (PPM) albo wczytaj JSON."),[i,a]=w.useState(null),r=w.useCallback(()=>{const m=T("report");m?.panels&&(t(m),n(null),a(m.meta?.loadedAt?`Odświeżono ${new Date(m.meta.loadedAt).toLocaleTimeString("pl-PL")}`:"Odświeżono"))},[]),l=w.useCallback(()=>{const m=c?.scope;if(!m){a("Brak zakresu — otwórz raport ponownie z CAD.");return}a("Pobieram z CAD…"),A("report",m),window.setTimeout(()=>r(),400)},[c?.scope,r]);w.useEffect(()=>{const m=u=>{u.key===$.report&&u.newValue&&r()};window.addEventListener("storage",m);let h=null;try{h=new BroadcastChannel(D),h.onmessage=u=>{const o=u.data;o?.type===H&&o.module==="report"&&r()}}catch{}return()=>{window.removeEventListener("storage",m);try{h?.close()}catch{}}},[r]),w.useEffect(()=>{const m=()=>{c?.meta?.sourceId!=="cad"&&c?.meta?.sourceId!=="session"||c?.scope&&A("report",c.scope)};return window.addEventListener("focus",m),()=>window.removeEventListener("focus",m)},[c?.meta?.sourceId,c?.scope]);const s=w.useMemo(()=>{if(!c?.panels)return null;try{return W(c)}catch{return null}},[c]),p=(m,h)=>{try{const u=Z(m,h);I("report",u),t(u),n(null),a(null)}catch(u){n(u?.message||"Niepoprawny JSON raportu")}},b=!!c?.scope&&(c.meta?.sourceId==="cad"||c.meta?.sourceId==="session");return k.jsxs("div",{style:{display:"flex",flexDirection:"column",height:"100%"},children:[k.jsx(j,{module:"report",title:"Raport",originLabel:c?.meta.originLabel||c?.scope.name,onLoadJson:p,onRefreshFromCad:b?l:void 0,statusHint:i||void 0}),e&&!c&&k.jsx("div",{style:{padding:24,color:"#fbbf24"},children:e}),c&&!s&&k.jsx("div",{style:{padding:24,color:"#f87171"},children:"Nie udało się wygenerować raportu HTML."}),s&&k.jsx("iframe",{title:"Raport projektu",srcDoc:s,style:{flex:1,minHeight:0,width:"100%",border:"none",background:"#f4f7f9"}})]})}R.createRoot(document.getElementById("report-root")).render(k.jsx(O.StrictMode,{children:k.jsx(q,{})}));
